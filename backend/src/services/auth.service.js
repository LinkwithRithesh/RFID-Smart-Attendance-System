...
async function checkDuplicates(payload) {
  ...
  // 3. Enrollment Number / Employee ID check
  const roleUpper = (payload.role || 'STUDENT').toUpperCase();

  if (roleUpper === 'STUDENT') {
    const regId = payload.enrollmentNumber;
    if (!regId) throw new ApiError(400, 'Enrollment number is required.');
    const existingRoll = await prisma.studentProfile.findUnique({ where: { enrollmentNumber: String(regId).trim() } });
    if (existingRoll) {
      throw new ApiError(409, 'Enrollment number already registered.');
    }
  } else if (roleUpper === 'FACULTY') {
    // Faculty doesn't require regId anymore during registration, just return
  }
}
...
async function verifyRegisterOtp(email, inputOtp) {
  ...
  if (roleUpper === 'STUDENT') {
    regId = String(payload.enrollmentNumber).trim();
    ...
    regId = 'PENDING-' + crypto.randomUUID().slice(0, 8);
  ...
  const passwordToUse = crypto.randomUUID(); // Assigned properly when admin approves
