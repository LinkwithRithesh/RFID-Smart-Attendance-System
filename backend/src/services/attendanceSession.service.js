...
async function openManualSession({ departmentId, subjectId, facultyId, durationMinutes, isEmergency }, actor, now = new Date()) {
  const overrideType = isEmergency
    ? 'EMERGENCY'
    : actor.role === 'ADMINISTRATOR' || actor.role === 'ADMIN'
      ? 'ADMIN'
      : actor.role;
  ...
  if (!finalSubjectId || !finalFacultyId) {
    const dayOfWeek = dayOfWeekFor(now);
    const timeOfDay = toTimeOfDay(now);
    const slot = await timetableRepository.findSlotCoveringNow(departmentId, dayOfWeek, timeOfDay);
    if (slot) {
      if (!finalSubjectId) finalSubjectId = slot.subjectId;
      if (!finalFacultyId) finalFacultyId = slot.facultyId;
    }
  }

  const session = await sessionRepository.create({
    sessionType: finalSubjectId ? 'CLASS' : 'EVENT',
    subjectId: finalSubjectId || null,
    facultyId: finalFacultyId || null,
    departmentId,
    sessionDate: now,
...
async function closeSessionManually(sessionId, actor) {
  ...
  const effectiveRole = actor.role === 'ADMINISTRATOR' || actor.role === 'ADMIN' ? 'ADMIN' : actor.role;
  if (session.overrideType !== 'NONE' && session.overrideType !== effectiveRole && !outranks(effectiveRole, session.overrideType)) {
