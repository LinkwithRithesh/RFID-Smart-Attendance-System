...
// Compute classes falling within a date range for a student
router.get('/compute-classes', async (req, res, next) => {
  try {
    ...
    if (!user || !user.departmentId || !user.studentProfile) {
      throw new ApiError(403, 'Only students with a valid department and profile can compute classes');
    }

    const departmentId = user.departmentId;
    const semester = user.studentProfile.currentSemester;
    ...
    while (currentDate <= end) {
      const day = dayOfWeekFor(currentDate);
      if (slotsByDay[day]) {
    ...
    // sort classes by date and startTime
    classes.sort((a, b) => {
      if (a.date !== b.date) return a.date.localeCompare(b.date);
      return new Date(a.startTime).getTime() - new Date(b.startTime).getTime();
    });

    return success(res, 200, 'Computed classes successfully', { classes });
  } catch (err) {
    next(err);
  }
});
