...
export default function ODRequestsPage() {
  ...
  const handleSubmitOD = async (e: React.FormEvent) => {
    e.preventDefault();
    await apiClient.post("/od-requests", { 
      category, 
      fromDate: date, 
      toDate: endDate, 
      subject, 
              ...
              <form onSubmit={handleSubmitOD} className="space-y-3 text-xs">
                ...
                <div className="flex justify-between items-end gap-2">
                  <div className="flex-1">
                    <label className="block text-slate-700 font-bold mb-1">Classes Affected</label>
                    <input
                      type="text"
                      required
                      value={subject}
                      ...
                      className="w-full p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-800"
                      placeholder="e.g. EC23C13, MA23C03"
                    />
                  </div>
                  <button
                    type="button"
                    onClick={async () => {
                      try {
                        ...
                        for(let d = start; d <= end; d.setDate(d.getDate() + 1)) {
                          ...
                          schedules.filter((s: any) => s.dayOfWeek === dayKey).forEach((s: any) => affected.add(s.subject?.code || s.subjectId));
                        }
                        
                        setSubject(Array.from(affected).filter(Boolean).join(", "));
                      } catch (err) {
                        console.error(err);
                        alert("Failed to compute classes");
                      }
                    }}
                    className="px-3 py-2 bg-[#EEF2F8] hover:bg-blue-100 text-[#0B2C5C] font-bold rounded-xl text-xs mb-0.5"
                  >
                    Compute Classes
                  </button>
                </div>
