...
export default function LiveAttendancePage() {
  ...
  useEffect(() => {
    const sid = typeof window !== 'undefined' ? localStorage.getItem('activeSessionId') : null;
    apiClient.get('/attendance-sessions/active').then((res: any) => {
      if (res.data && Array.isArray(res.data)) {
        const found = sid ? res.data.find((s: any) => String(s.id) === String(sid)) : res.data[0];
        if (found) {
  ...
  const totalEnrolled = activeSessionInfo?.totalEnrolled || 60;
  ...
  const handleCloseSession = async () => {
    ...
    try {
      const res = await apiClient.post(`/attendance-sessions/${activeSessionInfo.id}/close`);
      if (res.data?.success) {
        setSessionActive(false);
        setActiveSessionInfo(null);
        localStorage.removeItem('activeSessionId');
  ...
  const handleManualOverride = (e: React.FormEvent) => {
    ...
    setPresentCount((c) => Math.min((activeSessionInfo?.totalEnrolled || 60), c + 1));

    const activeSessionId = typeof window !== "undefined" ? localStorage.getItem("activeSessionId") || "1" : "1";
    apiClient.post("/attendance/manual", { userId: manualRoll, status: "PRESENT", sessionId: Number(activeSessionId) }).catch(()=>{});
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-xs font-mono font-bold text-[#0B2C5C] bg-[#EEF2F8] px-2 py-0.5 rounded">
                  {activeSessionInfo?.courseCode || "No Code"} • {activeSessionInfo?.room || "No Room"}
                </span>
            ...
            >
              ...
              <button
                ...
                className="px-4 py-2 rounded-full bg-rose-600 hover:bg-rose-700 text-white font-black text-xs flex items-center space-x-1.5 shadow-md transition-all"
              >
                <span>✖</span>
