...
function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiClient.get("/attendance/subjects")
              ...
              {todayClasses?.map((item, idx) => {
                ...
                return (
                  <tr key={idx} className={`hover:bg-slate-50 transition-colors font-medium ${borderAccent}`}>
                    <td className="p-3 font-mono font-bold text-[#0B2C5C]">{item.period || idx + 1}</td>
                    <td className="p-3">
...
function FacultyDashboard() {
  ...
  const [todayData, setTodayData] = useState<{ timetable: any[], activeClass: any | null }>({ timetable: [], activeClass: null });
  ...
  useEffect(() => {
    ...
    apiClient.get<any>("/dashboard/faculty/today")
      .then(res => { if (res.data) { setTodayData(res.data); if (res.data.activeClass?.id) localStorage.setItem("activeSessionId", res.data.activeClass.id); } })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {todayData.activeClass ? (
        <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-xs space-y-4 border-l-4 border-l-emerald-500">
          <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-3">
            <div className="flex items-center space-x-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0]">
            <div className="md:col-span-12 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <div className="text-sm font-extrabold text-[#0B2C5C] mt-0.5">{todayData.activeClass.code} {todayData.activeClass.subject}</div>
                <div className="text-xs text-slate-500 font-mono">Room {todayData.activeClass.room}</div>
              </div>
              <div>
                <div className="text-[10px] font-bold text-slate-500 uppercase">Enrolled Students</div>
                <div className="text-2xl font-black text-[#0B2C5C] mt-0.5 font-mono">{todayData.activeClass.enrolled}</div>
              </div>
              <div>
                ...
                <div className="text-2xl font-black text-emerald-600 mt-0.5 font-mono">
                  {todayData.activeClass.present} <span className="text-xs font-bold text-[#EF4444]">/ {todayData.activeClass.absent} Absent</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-xs flex items-center justify-between">
            <div className="text-slate-500 text-sm font-bold">No active live session currently.</div>
            <Link
                href="/live-attendance"
                className="px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-[#0B2C5C] font-bold text-xs shadow-xs"
            >
                Start a new session
            </Link>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left 7 cols: Today's Timetable */}
        <div className="lg:col-span-7 bg-white rounded-2xl border border-[#E2E8F0] p-5 shadow-xs space-y-4">
          <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-3">
            <div className="flex items-center space-x-2">
              <CalendarCheck className="w-4 h-4 text-[#0B2C5C]" />
              <h3 className="text-sm font-extrabold text-[#0B2C5C] uppercase tracking-tight">
                Your Schedule Today
              </h3>
            </div>
            <span className="text-xs font-mono text-emerald-600 font-bold">{todayData.timetable.length} Classes</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {todayData.timetable.length > 0 ? todayData.timetable.map((item: any, idx: number) => (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors font-medium">
                        <td className="p-3">
                        <div className="font-bold text-slate-900">{item.subject}</div>
                        <div className="text-[10px] font-mono text-slate-500">{item.code} - Semester {item.semester}</div>
                        </td>
                        <td className="p-3 font-mono font-semibold">{item.room}</td>
                        <td className="p-3 font-mono text-slate-600">{item.time}</td>
                    </tr>
                    ...
                    <tr>
                        <td colSpan={3} className="p-4 text-center text-slate-500">No classes scheduled for today.</td>
                    </tr>
                )}
              </tbody>
            </table>
        ...
        {/* Right 5 cols: Faculty Assigned Courses */}
        <div className="lg:col-span-5 bg-white rounded-2xl border border-[#E2E8F0] p-5 shadow-xs space-y-4">
          <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-3">
            <h3 className="text-sm font-extrabold text-[#0B2C5C] tracking-tight uppercase">
              Assigned Courses
            </h3>
            <span className="text-xs font-mono text-slate-500">{courses.length} Courses</span>
          </div>

          <div className="grid grid-cols-1 gap-4">
            {courses.length > 0 ? (
              courses.map((c) => (
                <div key={c.id} className="rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] overflow-hidden group hover:border-[#0B2C5C] transition-all p-3 space-y-1">
                    <div className="flex justify-between items-start">
                        <div className="font-extrabold text-slate-900 text-xs">{c.name}</div>
                        <div className="px-2 py-0.5 rounded bg-white text-[#0B2C5C] font-mono text-[10px] font-bold shadow-xs border border-slate-200">
                        {c.code}
                        </div>
                    </div>
                    <div className="text-[11px] text-slate-500">Semester {c.semester} • {c.totalSessions} Sessions Hosted</div>
                    <div className="flex justify-between items-center text-[10px] text-emerald-700 font-bold pt-1">
                      <span>Avg Attendance: {c.attendanceRate}%</span>
                    </div>
                </div>
              ))
            ) : (
