...
export default function AdminManagementPage() {
  ...
  const [approvePayload, setApprovePayload] = useState({ rollNumber: "", employeeId: "", designation: "", rfidCardId: "" });
  const [actionPendingItem, setActionPendingItem] = useState<any>(null);
  ...
  const loadFaculty = useCallback(async () => {
    ...
    try {
      const res = await api.getUsers({ role: "FACULTY", limit: 100 });
  ...
  const handleOpenApproveModal = (item: any) => {
    setActionPendingItem(item);
    setApprovePayload({ rollNumber: "", employeeId: "", designation: "", rfidCardId: "" });
    setShowApproveModal(true);
  };

  const handleConfirmApprove = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!actionPendingItem) return;
    setSubmitting(true);
    try {
      await api.approveRegistration(actionPendingItem.id, approvePayload);
      await loadPending();
      await loadStudents();
      await loadFaculty();
  ...
  return (
    <DashboardShell allowedRoles={["ADMIN"]}>
      <div className="space-y-6 font-sans text-slate-800">
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-2xl max-w-md w-full shadow-2xl p-6 space-y-4 text-slate-800">
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="pt-3 flex items-center justify-end space-x-2 border-t border-slate-100">
                  ...
                  <button
                    onClick={() => handleOpenApproveModal(selectedPendingItem)}
        ...
        {showApproveModal && actionPendingItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#071E40]/60 backdrop-blur-xs p-4">
            <div className="bg-white border border-[#E2E8F0] rounded-2xl max-w-sm w-full shadow-2xl p-6 space-y-4 text-slate-800">
              <div className="flex items-center space-x-2 text-emerald-600 border-b border-slate-100 pb-2">
                <CheckCircle2 className="w-5 h-5 shrink-0" />
                <h3 className="text-sm font-black text-slate-900">Approve Registration</h3>
              </div>

              <form onSubmit={handleConfirmApprove} className="space-y-4 text-xs">
                <p className="text-slate-600 leading-relaxed">
                  Please provide the required assignment details to approve this {actionPendingItem.role?.name || actionPendingItem.role} account.
                </p>

                {(actionPendingItem.role?.name === 'STUDENT' || actionPendingItem.role === 'STUDENT') ? (
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Roll Number *</label>
                    <input
                      type="text"
                      required
                      value={approvePayload.rollNumber}
                      onChange={(e) => setApprovePayload({...approvePayload, rollNumber: e.target.value})}
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50"
                  ...
                  <>
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        value={approvePayload.employeeId}
                        onChange={(e) => setApprovePayload({...approvePayload, employeeId: e.target.value})}
                        className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50"
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={approvePayload.designation}
                        onChange={(e) => setApprovePayload({...approvePayload, designation: e.target.value})}
                        className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">RFID Card ID *</label>
                  <input
                    type="text"
                    required
                    value={approvePayload.rfidCardId}
                    onChange={(e) => setApprovePayload({...approvePayload, rfidCardId: e.target.value})}
                    className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50"
                ...
                <div className="flex justify-end space-x-2 pt-2 border-t border-slate-100">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition-colors disabled:opacity-50 inline-flex items-center"
                  >
                    Confirm Approval
                  </button>
                </div>
