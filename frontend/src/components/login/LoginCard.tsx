...
export const LoginCard: React.FC = () => {
  ...
  // OTP Step State: "FORM" | "CAMERA" | "OTP" | "CONFIRMATION"
  const [regStep, setRegStep] = useState<"FORM" | "CAMERA" | "OTP" | "CONFIRMATION">("FORM");
  ...
  // Step 1: Validate Form and go to CAMERA
  ...
  // Step 1.5: Submit Face and Registration Form to Generate OTP
  const handleSendRegisterOtpWithFace = async () => {
    setError(null);
    if (!capturedFace) {
      setError("Please capture your face before proceeding.");
      return;
    }
    ...
    try {
      // 1. Upload face
      const response = await fetch(capturedFace);
      const blob = await response.blob();
      const faceUploadRes = await api.uploadFace(blob);
      ...
      const payload: any = {
        ...
        faceFileName: faceUploadRes.faceFileName,
      };

      setFaceFileName(faceUploadRes.faceFileName);

      if (regRole === "STUDENT") {
        payload.enrollmentNumber = regEnrollmentNumber.trim();
        payload.dob = regDob;
        payload.gender = regGender;
        payload.parentName = regParentName.trim();
        payload.parentPhone = regParentPhone.trim();
        ...
        payload.specialization = regSpecialization.trim();
  ...
  const handleVerifyRegisterOtp = async (e: React.FormEvent) => {
    ...
    setSubmittingReg(true);
    try {
      const res = await api.verifyRegisterOtp(`${regMobile.trim()}@temp.local`, otpInput.trim());
    ...
    <div className="w-full max-w-md bg-white border border-[#E2E8F0] rounded-2xl shadow-xl p-6 sm:p-8 font-sans text-slate-800">
      ...
      {mainTab === "REGISTER" && (
        <div>
          {/* STEP 1: FORM INPUT */}
          {regStep === "FORM" && (
            <form onSubmit={handleProceedToCamera} className="space-y-3.5 text-xs">
              {/* Role Selector */}
              <div>
              ...
              {regRole === "STUDENT" ? (
                <>
                  <div>
                    <label className="block text-slate-700 font-bold mb-1">Enrollment Number *</label>
                    <input type="text" required value={regEnrollmentNumber} onChange={(e) => setRegEnrollmentNumber(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-slate-900 focus:outline-none focus:border-[#0B2C5C]" placeholder="e.g. ENR2026..." />
                  </div>
                  <div className="grid grid-cols-2 gap-2.5">
                    <div>
                      <label className="block text-slate-700 font-bold mb-1">Date of Birth *</label>
                      <input type="date" required value={regDob} onChange={(e) => setRegDob(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-slate-900 focus:outline-none focus:border-[#0B2C5C]" />
                    </div>
                    <div>
                      <label className="block text-slate-700 font-bold mb-1">Gender *</label>
                      <select required value={regGender} onChange={(e) => setRegGender(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-slate-900 focus:outline-none focus:border-[#0B2C5C]">
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <div>
                      ...
                      <input type="text" required value={regParentName} onChange={(e) => setRegParentName(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-slate-900 focus:outline-none focus:border-[#0B2C5C]" />
                    </div>
                    <div>
                      <label className="block text-slate-700 font-bold mb-1">Parent Mobile *</label>
                      <input type="tel" required value={regParentPhone} onChange={(e) => setRegParentPhone(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-slate-900 focus:outline-none focus:border-[#0B2C5C]" />
                    </div>
                  </div>
                </>
              ) : (
                <div>
                  ...
                  <input type="text" value={regSpecialization} onChange={(e) => setRegSpecialization(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-slate-900 focus:outline-none focus:border-[#0B2C5C]" placeholder="e.g. AI / ML" />
                </div>
              )}
          ...
          {regStep === "CAMERA" && (
            <div className="space-y-4 text-xs">
              <div className="text-center mb-2">
                ...
                <p className="text-slate-500 text-[10px]">Please capture a clear photo of your face.</p>
              </div>

              <div className="relative w-full bg-slate-900 rounded-xl overflow-hidden aspect-video flex items-center justify-center">
                {!capturedFace ? (
                  <video
                    id="camera-preview"
                    autoPlay
                  ...
                  <img src={capturedFace} alt="Captured Face" className="w-full h-full object-cover" />
                )}
              ...
              <div className="flex space-x-2">
                {!capturedFace ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (video && video.srcObject) {
                        ...
                        if (ctx) {
                          ...
                          setCapturedFace(canvas.toDataURL("image/jpeg", 0.9));
                    ...
                    className="flex-1 py-3 px-4 rounded-full bg-[#0B2C5C] hover:bg-[#082046] text-white font-black text-sm flex items-center justify-center space-x-2 shadow-md transition-all"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 py-3 px-4 rounded-full bg-slate-200 hover:bg-slate-300 text-slate-800 font-black text-sm flex items-center justify-center space-x-2 transition-all"
                  >
              ...
              <div className="flex items-center justify-between text-[11px] mt-4">
                ...
                <button
                  type="button"
                  disabled={!capturedFace || submittingReg}
                  onClick={handleSendRegisterOtpWithFace}
                  className="text-[#F4573C] font-bold hover:underline disabled:opacity-50"
                >
                  {submittingReg ? "Uploading..." : "Submit & Send OTP →"}
                </button>
              </div>
            </div>
          )}
