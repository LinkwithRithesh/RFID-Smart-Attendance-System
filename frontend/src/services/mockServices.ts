  ...
  public async syncFromBackend() {
    ...
    try {
      ...
      var userRole = JSON.parse(localStorage.getItem("user") || "{}").role;
        const devRes = (userRole === "ADMIN" || userRole === "ADMINISTRATOR") ? await apiClient.get<any>("/devices") : { data: [] };
      ...
      // 4. Fetch Audit Logs
      var userRole = JSON.parse(localStorage.getItem("user") || "{}").role;
        const auditRes = (userRole === "ADMIN" || userRole === "ADMINISTRATOR") ? await apiClient.get<{ logs: any[] }>("/audit-logs") : { data: { logs: [] } };
