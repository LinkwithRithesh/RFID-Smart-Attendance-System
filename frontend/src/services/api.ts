...
export const BACKEND_TO_UI_ROLE_MAP: Record<string, "STUDENT" | "FACULTY" | "ADMIN"> = {
  ADMINISTRATOR: "ADMIN",
  SYSTEM_ADMIN: "ADMIN",
  FACULTY: "FACULTY",
  HOD: "FACULTY",
...
export const api = {
  ...
  async uploadFace(file: Blob) {
    const formData = new FormData();
    formData.append("faceImage", file, "face.jpg");
    
    // apiFetch assumes JSON unless headers are overridden.
    ...
    if (!res.ok || !data.success) throw new Error(data.message || "Failed to upload face image");
    return { success: true, faceFileName: data.data.faceFileName };
  },
  ...
  async approveRegistration(id: number, payload?: { rollNumber?: string; employeeId?: string; designation?: string; rfidCardId?: string; }) {
  ...
  async getCoursesByDepartment(departmentId?: number) {
    ...
    return { success: true, data: data.data as { id: number; code: string; name: string; departmentId?: number; department?: string; durationSemesters?: number; credits?: number; semester?: number }[] };
  },
  ...
  async getSubjects(courseId?: number) {
    ...
    const data = await apiFetch(`/subjects${query}`);
    return { success: true, data: data.data };
  },

  async getSubjectsByFaculty(facultyId: number) {
    const data = await apiFetch(`/subjects?facultyId=${facultyId}`);
    return { success: true, data: data.data };
  },
  ...
  async deleteSubject(id: number) {
    const data = await apiFetch(`/subjects/${id}`, { method: "DELETE" });
    return { success: true, message: data.message };
  },
